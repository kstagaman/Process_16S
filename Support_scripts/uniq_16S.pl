...
my $help;
my $quiet;
my $size; # whether to include the string ';size=' before the abundance ( for vsearch ) or not ( for uclust )
my $print_unmatched;
my $outDir = './';
my $inFile;
my $keep = 1; 
...
if ( $help ) {
    ...
    print "\t\t-s: include the string ';size=' before the abundance for vsearch, omit for uclust\n";
    ...
    if ( $size ) {
        $spacer = ";size=";
        $hdr_style = "vsearch_hdrs";
    } else {
        $spacer = " ";
        $hdr_style = "uclust_hdrs";
    }
    my ( $outName ) = $inFile =~ /(.+)\.fasta\.*g*z*$/;
    my $gt = $keep - 1; 
