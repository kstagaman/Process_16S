...
# Alternatively, will return predicted occurrence frequencies for each OTU based on their abundance in the metacommunity when stats=FALSE. For use in R.
...
# 	m.mle <- mle(sncm.LL, start=list(m=0.001, sigma=0.01), nobs=length(p))
...
# 	bic.fit <- BIC(m.mle)
	...
	if(stats==TRUE){
		fitstats <- data.frame(m=numeric(), 
                               ...
                               Richness=numeric(), 
                               Detect=numeric()) 
                               # maxLL=numeric(), AIC=numeric(), BIC=numeric(), m.mle=numeric() 
		fitstats[1,] <- c(coef(m.fit), 
                          coef(m.fit)-m.ci[1], 
                          ...
                          length(p), 
                          d) 
                          # , m.mle@coef['m'] , aic.fit , bic.fit , m.mle@details$values
		return(fitstats)
	} else {
